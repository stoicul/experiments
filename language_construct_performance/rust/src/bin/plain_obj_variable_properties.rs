...
fn main() {
    ...
    println!("Starting Plain Variable Properties Benchmark with {} entries...", num_entries);
    ...
    println!("\n--- CREATION ---");
    benchmark_stats("Plain Creation", &mut stats, "creationTimeMs", true, &GLOBAL, || {
    ...
    println!("\n--- PLAIN TRAVERSAL ---");
    benchmark_stats("Plain Traversal (Plain)", &mut stats, "plainTraversalTimeMs", false, &GLOBAL, || {
    ...
    println!("\n--- PROPERTY ACCESS ---");
    benchmark_stats("Property Access (Plain)", &mut stats, "propAccessTimeMs", false, &GLOBAL, || {
        let mut sum: i64 = 0;
        for i in 0..num_entries {
    ...
    println!("\n--- FILTERING ---");
    benchmark_stats("Filtering (Plain)", &mut stats, "filterTimeMs", false, &GLOBAL, || {
        let mut matched = 0;
        for i in 0..num_entries {
            let map = plain_array[i].as_ref().unwrap();
            let details = map.get("details").unwrap().as_object().unwrap();
    ...
    println!("\n--- MUTATION ---");
    benchmark_stats("Mutation (Plain)", &mut stats, "mutationTimeMs", false, &GLOBAL, || {
        for i in 0..num_entries {
    ...
    println!("\n--- DELETE PROPERTY ---");
    benchmark_stats("Delete Property (Plain)", &mut stats, "deletePropertyTimeMs", false, &GLOBAL, || {
        for i in 0..num_entries {
            let map = plain_array[i].as_mut().unwrap();
            let details = map.get_mut("details").unwrap().as_object_mut().unwrap();
            details.remove("ud");
    ...
    save_stats("stats_variable.json", "plain object", stats);
}
