...
fn main() {
    ...
    println!("Starting Value Object Idiomatic Fixed Properties Benchmark with {} entries...", num_entries);

    // Array of Box to simulate pointers and heap allocation similar to Go's pointers
    let mut value_obj_array: Vec<ValueObjectNode> = Vec::with_capacity(num_entries);

    println!("\n--- CREATION ---");
    benchmark_stats("Value Object Idiomatic Creation", &mut stats, "creationTimeMs", true, &GLOBAL, || {
        for i in 0..num_entries {
            value_obj_array.push(create_value_object_fixed(i));
        }
    });
    ...
    benchmark_stats("Property Access (Value Object Idiomatic)", &mut stats, "propAccessTimeMs", false, &GLOBAL, || {
        let mut sum: i64 = 0;
        for i in 0..num_entries {
            sum += value_obj_array[i].details.ag.a.s.r as i64;
        }
        sum
    });
    ...
    benchmark_stats("Delete Property (Value Object Idiomatic)", &mut stats, "deletePropertyTimeMs", false, &GLOBAL, || {
        for i in 0..num_entries {
            value_obj_array[i].details.ud = 0; // In Go structs we can't truly delete, set to zero value
        }
    });
