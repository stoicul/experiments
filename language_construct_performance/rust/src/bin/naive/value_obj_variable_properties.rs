...
fn main() {
    ...
    println!("Starting Value Object Naive Variable Properties Benchmark with {} entries...", num_entries);

    let mut value_obj_array: Vec<Option<Box<ValueObjectNodeVar>>> = Vec::with_capacity(num_entries);
    for _ in 0..num_entries {
        value_obj_array.push(None);
    }

    println!("\n--- CREATION ---");
    benchmark_stats("Value Object Naive Creation", &mut stats, "creationTimeMs", true, &GLOBAL, || {
        for i in 0..num_entries {
            value_obj_array[i] = Some(Box::new(create_value_object_variable(i)));
        }
    });

    println!("\n--- PLAIN TRAVERSAL ---");
    benchmark_stats("Plain Traversal (Value Object Naive)", &mut stats, "plainTraversalTimeMs", false, &GLOBAL, || {
    ...
    println!("\n--- PROPERTY ACCESS ---");
    benchmark_stats("Property Access (Value Object Naive)", &mut stats, "propAccessTimeMs", false, &GLOBAL, || {
        let mut sum: i64 = 0;
        for i in 0..num_entries {
            if let Some(ag) = &value_obj_array[i].as_ref().unwrap().details.ag {
                sum += ag.a.s.r as i64;
            }
        }
        sum
    ...
    println!("\n--- FILTERING ---");
    benchmark_stats("Filtering (Value Object Naive)", &mut stats, "filterTimeMs", false, &GLOBAL, || {
        let mut matched = 0;
        for i in 0..num_entries {
    ...
    println!("\n--- MUTATION ---");
    benchmark_stats("Mutation (Value Object Naive)", &mut stats, "mutationTimeMs", false, &GLOBAL, || {
        for i in 0..num_entries {
    ...
    println!("\n--- DELETE PROPERTY ---");
    benchmark_stats("Delete Property (Value Object Naive)", &mut stats, "deletePropertyTimeMs", false, &GLOBAL, || {
        for i in 0..num_entries {
            value_obj_array[i].as_mut().unwrap().details.ud = 0;
        }
    });

    drop(value_obj_array);

    save_stats("stats_variable.json", "value object naive", stats);
}
