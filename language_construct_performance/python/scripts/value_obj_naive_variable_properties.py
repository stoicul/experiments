...
def run_benchmark():
    stats = {}
    print(f"Starting Value Object Naive Variable Properties Benchmark with {NUM_ENTRIES:,} entries...\n")
    ...
    benchmark_stats("Value Object Naive Creation", stats, "creationTimeMs", creation, track_memory=True)
    ...
    def traversal():
        ...
        for i in range(NUM_ENTRIES):
            if value_obj_list[i] is not None:
                dummy_count += 1
        return dummy_count
    benchmark_stats("Plain Traversal (Value Object Naive)", stats, "plainTraversalTimeMs", traversal)
    ...
    def property_access():
        ...
        for i in range(NUM_ENTRIES):
            ...
            if ag is not None and ag.a is not None and ag.a.s is not None:
                sum_val += ag.a.s.r
        return sum_val
    benchmark_stats("Property Access (Value Object Naive)", stats, "propAccessTimeMs", property_access)
    ...
    def filtering():
        ...
        for i in range(NUM_ENTRIES):
            if value_obj_list[i].details.la > 1772455500:
                matched += 1
        return matched
    benchmark_stats("Filtering (Value Object Naive)", stats, "filterTimeMs", filtering)

    print("\n--- MUTATION ---")
    def mutation():
        for i in range(NUM_ENTRIES):
            value_obj_list[i].details.la += 1
    benchmark_stats("Mutation (Value Object Naive)", stats, "mutationTimeMs", mutation)
    ...
    def delete_property():
        ...
        for i in range(NUM_ENTRIES):
            try:
                ...
                pass
    benchmark_stats("Delete Property (Value Object Naive)", stats, "deletePropertyTimeMs", delete_property)
