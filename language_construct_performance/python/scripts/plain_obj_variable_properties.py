...
def run_benchmark():
    stats = {}
    print(f"Starting Plain Variable Properties Benchmark with {NUM_ENTRIES:,} entries...\n")
    ...
    benchmark_stats("Plain Creation", stats, "creationTimeMs", creation, track_memory=True)
    ...
    def traversal():
        ...
        for i in range(NUM_ENTRIES):
            if plain_list[i] is not None:
                dummy_count += 1
        return dummy_count
    benchmark_stats("Plain Traversal (Plain)", stats, "plainTraversalTimeMs", traversal)
    ...
    def property_access():
        ...
        for i in range(NUM_ENTRIES):
            ...
            if ag:
                sum_val += ag['a']['s']['r']
        return sum_val
    benchmark_stats("Property Access (Plain)", stats, "propAccessTimeMs", property_access)
    ...
    def filtering():
        ...
        for i in range(NUM_ENTRIES):
            if plain_list[i]['details']['la'] > 1772455500:
                matched += 1
        return matched
    benchmark_stats("Filtering (Plain)", stats, "filterTimeMs", filtering)
    
    print("\n--- MUTATION ---")
    def mutation():
        for i in range(NUM_ENTRIES):
            plain_list[i]['details']['la'] += 1
    benchmark_stats("Mutation (Plain)", stats, "mutationTimeMs", mutation)
    ...
    benchmark_stats("Delete Property (Plain)", stats, "deletePropertyTimeMs", delete_property)
