...
func main() {
	...
	fmt.Printf("Starting Plain Idiomatic Variable Properties Benchmark with %d entries...\n", NumEntries)

	plainArray := make([]map[string]interface{}, NumEntries)

	fmt.Println("\n--- CREATION ---")
	BenchmarkStats("Plain Idiomatic Creation", stats, "creationTimeMs", true, func() interface{} {
	...
	fmt.Println("\n--- PLAIN TRAVERSAL ---")
	BenchmarkStats("Plain Traversal (Plain Idiomatic)", stats, "plainTraversalTimeMs", false, func() interface{} {
		dummyCount := 0
		for i := 0; i < NumEntries; i++ {
			if plainArray[i] != nil {
				dummyCount++
	...
	fmt.Println("\n--- PROPERTY ACCESS ---")
	BenchmarkStats("Property Access (Plain Idiomatic)", stats, "propAccessTimeMs", false, func() interface{} {
		sum := 0
		for i := 0; i < NumEntries; i++ {
	...
	fmt.Println("\n--- FILTERING ---")
	BenchmarkStats("Filtering (Plain Idiomatic)", stats, "filterTimeMs", false, func() interface{} {
		matched := 0
		for i := 0; i < NumEntries; i++ {
			details := plainArray[i]["details"].(map[string]interface{})
	...
	fmt.Println("\n--- MUTATION ---")
	BenchmarkStats("Mutation (Plain Idiomatic)", stats, "mutationTimeMs", false, func() interface{} {
		for i := 0; i < NumEntries; i++ {
			details := plainArray[i]["details"].(map[string]interface{})
	...
	fmt.Println("\n--- DELETE PROPERTY ---")
	BenchmarkStats("Delete Property (Plain Idiomatic)", stats, "deletePropertyTimeMs", false, func() interface{} {
		for i := 0; i < NumEntries; i++ {
			details := plainArray[i]["details"].(map[string]interface{})
			delete(details, "ud")
		}
