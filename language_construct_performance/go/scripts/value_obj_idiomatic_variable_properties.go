...
func main() {
	...
	fmt.Printf("Starting Value Object Idiomatic Variable Properties Benchmark with %d entries...\n", NumEntries)

	valueObjArray := make([]ValueObjectNodeVar, NumEntries)

	fmt.Println("\n--- CREATION ---")
	BenchmarkStats("Value Object Idiomatic Creation", stats, "creationTimeMs", true, func() interface{} {
	...
	fmt.Println("\n--- PLAIN TRAVERSAL ---")
	BenchmarkStats("Plain Traversal (Value Object Idiomatic)", stats, "plainTraversalTimeMs", false, func() interface{} {
		dummyCount := 0
		for i := 0; i < NumEntries; i++ {
			if valueObjArray[i].Id != "" {
				dummyCount++
			}
		}
		return dummyCount
	...
	fmt.Println("\n--- PROPERTY ACCESS ---")
	BenchmarkStats("Property Access (Value Object Idiomatic)", stats, "propAccessTimeMs", false, func() interface{} {
		sum := 0
		for i := 0; i < NumEntries; i++ {
	...
	fmt.Println("\n--- FILTERING ---")
	BenchmarkStats("Filtering (Value Object Idiomatic)", stats, "filterTimeMs", false, func() interface{} {
		matched := 0
		for i := 0; i < NumEntries; i++ {
	...
	fmt.Println("\n--- MUTATION ---")
	BenchmarkStats("Mutation (Value Object Idiomatic)", stats, "mutationTimeMs", false, func() interface{} {
		for i := 0; i < NumEntries; i++ {
	...
	fmt.Println("\n--- DELETE PROPERTY ---")
	BenchmarkStats("Delete Property (Value Object Idiomatic)", stats, "deletePropertyTimeMs", false, func() interface{} {
		for i := 0; i < NumEntries; i++ {
