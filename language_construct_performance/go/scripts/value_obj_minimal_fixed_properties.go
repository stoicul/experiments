...
type ValueObjectNodeMinimal struct {
	Label    string
	Id       string
	EdgeTo   []string
	AccessTo []string
...
func createValueObjectMinimalFixed(index int) ValueObjectNodeMinimal {
	return ValueObjectNodeMinimal{
...
func main() {
	...
	fmt.Printf("Starting Value Object Minimal Fixed Properties Benchmark with %d entries...\n", NumEntries)

	valueObjArray := make([]*ValueObjectNodeMinimal, NumEntries)

	fmt.Println("\n--- CREATION ---")
	BenchmarkStats("Value Object Minimal Creation", stats, "creationTimeMs", true, func() interface{} {
		for i := 0; i < NumEntries; i++ {
			obj := createValueObjectMinimalFixed(i)
			valueObjArray[i] = &obj
		}
		return nil
	})
	...
	BenchmarkStats("Property Access (Value Object Minimal)", stats, "propAccessTimeMs", false, func() interface{} {
		sum := 0
		for i := 0; i < NumEntries; i++ {
			ag := valueObjArray[i].Details["ag"].(map[string]interface{})
