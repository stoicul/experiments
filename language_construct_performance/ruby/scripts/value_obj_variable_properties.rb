...
puts "Starting Value Object Variable Properties Benchmark with #{NUM_ENTRIES.to_s.gsub(/(\d)(?=(\d{3})+$)/, '\\1,')} entries..."

value_obj_array = Array.new(NUM_ENTRIES)

puts "\n--- CREATION ---"
benchmark_stats("Value Object Creation", stats, "creationTimeMs", track_memory: true) do
...
puts "\n--- PLAIN TRAVERSAL ---"
benchmark_stats("Plain Traversal (Value Object)", stats, "plainTraversalTimeMs") do
  dummy_count = 0
  NUM_ENTRIES.times do |i|
...
puts "\n--- PROPERTY ACCESS ---"
benchmark_stats("Property Access (Value Object)", stats, "propAccessTimeMs") do
  sum = 0
  NUM_ENTRIES.times do |i|
...
puts "\n--- FILTERING ---"
benchmark_stats("Filtering (Value Object)", stats, "filterTimeMs") do
  matched = 0
  NUM_ENTRIES.times do |i|
...
puts "\n--- MUTATION ---"
benchmark_stats("Mutation (Value Object)", stats, "mutationTimeMs") do
  NUM_ENTRIES.times do |i|
...
puts "\n--- DELETE PROPERTY ---"
benchmark_stats("Delete Property (Value Object)", stats, "deletePropertyTimeMs") do
  NUM_ENTRIES.times do |i|
