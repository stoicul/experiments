...
$runBenchmark = function () use (&$stats) {
    echo sprintf("Starting Value Object Idiomatic Idiomatic Variable Properties Benchmark with %s entries...\n", number_format(NUM_ENTRIES));

    $valueObjArray = [];

    echo "\n--- CREATION ---\n";
    benchmarkStats("Value Object Idiomatic Creation", $stats, "creationTimeMs", function () use (&$valueObjArray) {
    ...
    echo "\n--- PLAIN TRAVERSAL ---\n";
    benchmarkStats("Plain Traversal (Value Object Idiomatic)", $stats, "plainTraversalTimeMs", function () use (&$valueObjArray) {
        $dummyCount = 0;
        for ($i = 0; $i < NUM_ENTRIES; $i++) {
    ...
    echo "\n--- PROPERTY ACCESS ---\n";
    benchmarkStats("Property Access (Value Object Idiomatic)", $stats, "propAccessTimeMs", function () use (&$valueObjArray) {
        $sum = 0;
        for ($i = 0; $i < NUM_ENTRIES; $i++) {
    ...
    echo "\n--- FILTERING ---\n";
    benchmarkStats("Filtering (Value Object Idiomatic)", $stats, "filterTimeMs", function () use (&$valueObjArray) {
        $matched = 0;
        for ($i = 0; $i < NUM_ENTRIES; $i++) {
    ...
    echo "\n--- MUTATION ---\n";
    benchmarkStats("Mutation (Value Object Idiomatic)", $stats, "mutationTimeMs", function () use (&$valueObjArray) {
        for ($i = 0; $i < NUM_ENTRIES; $i++) {
    ...
    echo "\n--- DELETE PROPERTY ---\n";
    benchmarkStats("Delete Property (Value Object Idiomatic)", $stats, "deletePropertyTimeMs", function () use (&$valueObjArray) {
        for ($i = 0; $i < NUM_ENTRIES; $i++) {
